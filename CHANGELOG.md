...
- **Security**: bumped `@vscode/vsce` (extension) and `vitest` (web) toolchains, resolving every Dependabot alert reachable through real dependency resolution at the time (`npm audit`: 0 vulnerabilities in `audiospectra-cli`; one remaining moderate finding in `web` is in `drizzle-kit`'s dev-only migration codepath with no non-breaking upstream fix - not reachable from anything that runs in production).
